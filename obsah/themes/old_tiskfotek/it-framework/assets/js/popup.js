(function($) { 
	...
	function load_icons(){
		...
		$('body').append('<div class="add_icon"></div><div id="icon-overlay"></div>');
		var dw = parseInt($(window).width())/2,
			mw = parseInt($('.add_icon').width())/2,
			dh = parseInt($(window).height())/2,
			mh = parseInt($('.add_icon').height())/2,
			lft = dw-mw+'px',
			tp = dh-mh+'px';
		$('.add_icon').css({left:lft,top:tp});
		click_icons();
		$.ajax({
			type: "GET",
	...
	function click_icons(){
		...
		$('.btn_icon').click(function(e){
			...
			$('.add_icon .icons_set').find('i').each(function(){
				$(this).click(function(){
					$('.add_icon .icons_set').find('i').removeClass('selectedI');
