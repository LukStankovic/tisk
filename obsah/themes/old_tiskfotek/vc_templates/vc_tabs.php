<?php
$output = $title = $interval = $tab_icon = $ico = $el_class = '';
...
$element = 'wpb_tabs';
if ( 'vc_tour' == $this->shortcode ) $element = 'wpb_tour tabs-vertical';
...
foreach ( $tab_titles as $tab ) {
    ...
    if(isset($tab_atts['the_icon'])) {
        $ico = '<i class="'.$tab_atts['the_icon'].'"></i>';
    }
    if(isset($tab_atts['title'])) {
        $tabs_nav .= '<li><a href="#tab-' . ( isset( $tab_atts['tab_id'] ) ? $tab_atts['tab_id'] : sanitize_title( $tab_atts['title'] ) ) . '">' .$ico. $tab_atts['title'] . '</a></li>';
    }
}
$tabs_nav .= '</ul>' . "\n";
$tab_sty = null;
...
$output .= "\n\t" . '<div class="' . $css_class . $tab_sty . '" data-interval="' . $interval . '">';
